...
# Imagen pineada a ubuntu:24.04: ubuntu:latest ya apunta a 26.04, cuyo coreutils
# reimplementado en Rust invierte el signo del experimento de hash.
set -e
...
if command -v docker &>/dev/null; then
    ...
    docker run -d --name "$DOCKER_CONTAINER" ubuntu:24.04 sleep 3600 > /dev/null 2>&1
    sleep 1
...
if command -v podman &>/dev/null; then
    ...
    podman run -d --name "$PODMAN_CONTAINER" ubuntu:24.04 sleep 3600 > /dev/null 2>&1
    sleep 1
